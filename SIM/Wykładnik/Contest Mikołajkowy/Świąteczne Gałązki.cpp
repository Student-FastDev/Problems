...
ull getRootedHash(int nodeIndex, int parentIndex, const vector<vector<int>>& neighbourList) {
    ull currentHash = 1;
    for (int neighbor : neighbourList[nodeIndex]) {
        if (neighbor != parentIndex) {
            currentHash += mix(getRootedHash(neighbor, nodeIndex, neighbourList));
        }
    }
    return currentHash;
}
...
ull getUnrootedHash(const vector<vector<int>>& neighbourList) {
...
void solveAll() {
    ...
    for (const auto& tree : allTrees) {
        ...
        for (int divisor : divisors) {
            ...
            fill(isChunkRoot.begin(), isChunkRoot.begin() + vertexCount + 2, false);
            ...
            if (partitionResult == 0) {
                ...
                for (int i = 1; i <= vertexCount; ++i) {
                    if (isChunkRoot[i]) {
                        ...
                        ull chunkHash = getUnrootedHash(chunkAdjacency);
                        if (firstChunk) {
                            commonHash = chunkHash;
                            firstChunk = false;
